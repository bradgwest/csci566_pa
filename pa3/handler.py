...
def send_messages(client, send_queue, num_messages, sizes, rate, delay=20):
    for size in sizes:
        for r in rate:
...
def receive_messages(client, receive_queue, send_queue=None, delete_queue=None,
    ...
    while True:
        ...
        for message in response['Messages']:
            ...
            if message_attributes and message_attributes.get('message_id'):
...
def perform_experiment(device, latency, loss, bandwidth, log_file, send_queue,
                       ...
                       comm_type, sender, num_clients, server, delay):
