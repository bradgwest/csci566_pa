...
BANDWIDTH=( 25 50 75 100 200 300 400 )
DURATION=( )
MEASURED_B=( )
for b in "${BANDWIDTH[@]}"
do
	...
	ts=$(date +%s%N)
	# Download the file
	scp -i aws_csci566_pa1.pem ubuntu@ec2-18-221-66-82.us-east-2.compute.amazonaws.com:~/file.txt .
	tt=$((($(date +%s%N) - $ts)/1000000))
	DURATION+=( $tt )

	# Measure the bandwidth
	iperf3 --logfile iperf3.log -c 18.221.66.82 
